...
must_haves:
  truths:
    ...
    - "v1.2 behavior preserved when smart selection returns low confidence"
...
Purpose: Connect the selection service to the rendering pipeline so arrays automatically render with context-appropriate components. Ensure v1.2 behavior is preserved (user overrides always win) and cards show only relevant fields (primary + secondary tiers).
...
<task type="auto">
  ...
  <action>
Wire the selection service into DynamicRenderer to provide smart defaults while preserving v1.2 behavior.
...
// AFTER (v1.3 with smart defaults):
const config = fieldConfigs[activePath]
const override = config?.componentType
...
- INT-01: Smart defaults must NOT break v1.2 behavior
...
**Important:** This is backward compatible. When importance prop is undefined, all fields display (v1.2 behavior).
...
- Hero image detection logic
  ...
  <done>
    ...
    - When importance undefined, all fields displayed (v1.2 compatibility)
...
3. Manual verification in browser:
   ...
   - Verify cards/tables render as before (v1.2 baseline)
...
5. Verify INT-01 (v1.2 behavior preserved):
...
- CardListRenderer shows all fields when importance prop undefined (v1.2 compat)
- Component switcher continues to work (INT-05)
- No breaking changes to existing v1.2 behavior (INT-01)
