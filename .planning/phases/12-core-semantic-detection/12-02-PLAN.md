...
**commerce.ts** - 4 patterns (use SemanticPattern type):
1. pricePattern:
...
**identity.ts** - 6 patterns (use SemanticPattern type):
1. emailPattern:
...
**media.ts** - 4 patterns (use SemanticPattern type):
1. imagePattern:
...
1. ratingPattern (SemanticPattern):
   - Names: /\b(rating|score|stars|puntuacion|note|bewertung)\b/i
   - Types: ['number']
   - Values: number between 0-5 or 0-10 or 0-100
   - Formats: 'float', 'double'

2. **reviewsPattern (MUST use CompositePattern type, not SemanticPattern):**
   ```typescript
   import { CompositePattern } from '../types'

   export const reviewsPattern: CompositePattern = {
     category: 'reviews',
     namePatterns: [{ regex: /\b(reviews?|comments?|feedback|opiniones|avis|bewertungen)\b/i, weight: 0.4, languages: ['en', 'es', 'fr', 'de'] }],
     ...
     requiredFields: [
       ...
       { nameRegex: /\b(comment|text|body|content|review)\b/i, type: 'string' }
     ],
     minItems: 1
   }
   ```
   This pattern requires BOTH a rating-like field AND a comment-like field in array items.

3. tagsPattern (SemanticPattern):
   - Names: /\b(tags?|labels?|categories?|keywords?|etiquetas?)\b/i
   - Types: ['array']
   - Values: array of strings
   - Formats: none

4. statusPattern (SemanticPattern):
   - Names: /\b(status|state|stage|estado|statut|zustand)\b/i
   - Types: ['string']
   - Values: common status values (active, inactive, pending, completed, etc.)
   - Formats: none

5. titlePattern (SemanticPattern):
   - Names: /\b(title|headline|subject|heading|titulo|titre|titel)\b/i
   - Types: ['string']
   - Values: non-empty string
   - Formats: none

6. descriptionPattern (SemanticPattern):
...
- Export getAllPatterns(): SemanticPattern[] (array of all ~22 patterns, excluding composite)
- Export getPattern(category: SemanticCategory): SemanticPattern | undefined
- Export getCompositePatterns(): CompositePattern[] (returns [reviewsPattern] and any future composite patterns)
...
Use multilingual name patterns with alternation: /\b(price|precio|prix|preis)\b/i
  ...
  <verify>
    ...
    - reviewsPattern uses CompositePattern type with requiredFields property
    - getCompositePatterns() returns array containing reviewsPattern
  </verify>
  <done>22 semantic patterns defined across 4 categories with multilingual support, reviewsPattern properly typed as CompositePattern</done>
</task>
...
1. Import:
   ...
   - Types from ./types (SemanticPattern, CompositePattern, ConfidenceResult)
...
3. detectCompositeSemantics function (for arrays):
   ...
   ```
   - Get composite patterns from getCompositePatterns()
   - For each CompositePattern:
     - Check if array name matches any namePattern
     - Check if itemFields satisfy ALL requiredFields (each requiredField.nameRegex matches some itemField.name AND types match)
     - Calculate confidence based on name match + structure match
...
4. getBestMatch helper:
   ...
   ```
   - Return first result if level is 'high' (confidence >= 0.75 per user decision)
   - Return null otherwise (below smart default threshold)
...
- Export getAllPatterns, getCompositePatterns for debugging/inspection
...
This is the public API for Phase 13+.
  ...
  <verify>
    ...
    - detectCompositeSemantics properly checks CompositePattern.requiredFields
  </verify>
  <done>Detection engine complete with memoized detectSemantics function and proper composite pattern support</done>
</task>
...
4. Composite detection works for array patterns using CompositePattern.requiredFields
5. reviewsPattern typed as CompositePattern with requiredFields for rating+comment
</verification>

<success_criteria>
- 22 semantic patterns defined with multilingual support
- detectSemantics function works for primitive fields
- detectCompositeSemantics function works for array fields using CompositePattern interface
- reviewsPattern uses CompositePattern type with requiredFields property
