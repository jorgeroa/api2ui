...
must_haves:
  truths:
    ...
    - "The component type selector in the popover IS the field-level switching mechanism — fields have fewer options and smaller visual space than top-level renderers, so they use a dropdown select instead of the carousel pattern (per user decision on field-level switching)"
