...
must_haves:
  truths:
    ...
    - "Global style overrides are persisted and applied at runtime"
    - "Per-endpoint style overrides are stored and persisted alongside global overrides"
  artifacts:
...
- State shape:
  ...
  - `endpointOverrides: Record<string, StyleOverrides>` (default: {}, keyed by endpoint operationId or path string)
...
- Actions (all using functional set() to avoid stale closures):
  ...
  - `setStyleOverride(key, value)` - set single CSS variable override on global styleOverrides
  - `setStyleOverrides(overrides: StyleOverrides)` - bulk set global overrides
  - `setEndpointStyleOverride(endpoint: string, key: string, value: string)` - set single CSS variable override for a specific endpoint. Creates the endpoint entry in endpointOverrides if it doesn't exist.
  - `clearEndpointOverrides(endpoint: string)` - remove all style overrides for a specific endpoint (delete the key from endpointOverrides)
  - `getEndpointStyleOverrides(endpoint: string)` - returns the merged result: global styleOverrides spread with endpointOverrides[endpoint] on top (endpoint values win). Returns just global styleOverrides if no endpoint overrides exist.
  - `resetConfig()` - clear all field configs, style overrides, and endpoint overrides
...
- Persist configuration:
  ...
  - `partialize`: exclude `mode` and `panelOpen` (ephemeral UI state). Include `endpointOverrides` in persisted state.
...
Write a quick sanity check: import useConfigStore, call setMode, toggleFieldVisibility, applyTheme, setEndpointStyleOverride - verify no runtime errors.
  </verify>
  <done>
configStore.ts exports useConfigStore with all listed actions including endpointOverrides and per-endpoint actions. Types are correctly defined. persist middleware is configured with partialize excluding mode/panelOpen but including endpointOverrides. TypeScript compiles cleanly.
  </done>
</task>
...
7. endpointOverrides field exists in store and is persisted
8. setEndpointStyleOverride and clearEndpointOverrides actions work
</verification>

<success_criteria>
- useConfigStore is importable and functional with all actions
- Persist middleware saves fieldConfigs, globalTheme, styleOverrides, and endpointOverrides to localStorage key 'api2ui-config'
...
- Per-endpoint style overrides can be set, retrieved (merged with global), and cleared
