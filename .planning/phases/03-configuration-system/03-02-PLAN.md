...
must_haves:
  truths:
    ...
    - "Endpoint-specific style overrides are applied when viewing a specific endpoint"
  artifacts:
    ...
    - path: "src/components/config/ThemeApplier.tsx"
      provides: "Applies theme class, global style overrides, and endpoint-specific overrides to document element"
  ...
  key_links:
    ...
    - from: "src/components/config/ThemeApplier.tsx"
      to: "src/store/configStore.ts"
      via: "useConfigStore for globalTheme, styleOverrides, and endpointOverrides"
      pattern: "useConfigStore"
    - from: "src/components/config/ThemeApplier.tsx"
      to: "src/store/appStore.ts"
      via: "useAppStore for selectedOperationIndex to determine current endpoint"
      pattern: "useAppStore|selectedOperationIndex"
    - from: "src/App.tsx"
...
Purpose: This creates the visual framework users interact with to enter configuration mode. The floating button is the entry point, the panel is the workspace for deeper settings, and the ThemeApplier connects CSS variables to the store -- including endpoint-specific style overrides.

Output: ConfigToggle (floating button), ConfigPanel (slide-out shell), ThemeApplier (CSS variable applicator with endpoint awareness), updated App.tsx.
</objective>
...
- Read `globalTheme`, `styleOverrides`, and `endpointOverrides` from configStore
- Read `selectedOperationIndex` and the operations list from appStore to determine the current endpoint identifier (use the operation's operationId or `method-path` string as the key)
- `useEffect` to apply `theme-{preset}` class to `document.documentElement`
- `useEffect` to compute the effective style overrides for the current view:
  1. Start with global `styleOverrides` as base
  2. If a specific endpoint is currently selected (selectedOperationIndex is set and valid), look up `endpointOverrides[endpointKey]`
  3. Merge endpoint overrides on top of global (endpoint values win): `{ ...styleOverrides, ...endpointOverrides[endpointKey] }`
  4. Apply each merged override as CSS custom property via `document.documentElement.style.setProperty(key, value)`
- Clean up old overrides when they change (remove properties no longer in the merged set)
- Dependencies for the useEffect: `globalTheme`, `styleOverrides`, `endpointOverrides`, `selectedOperationIndex`
...
Run `npm run build` - build succeeds.
  ...
  <done>
Three components created. ConfigToggle renders fixed-position gear button with mode toggle and hidden-field badge. ConfigPanel renders slide-out Dialog with section placeholders. ThemeApplier applies theme class and CSS variables to document, including endpoint-specific overrides merged on top of global when viewing a specific endpoint.
  </done>
</task>
...
Run `npm run build` to confirm production build succeeds.
  ...
  <done>
App.tsx renders ConfigToggle (floating button), ConfigPanel (slide-out), and ThemeApplier. Gear button toggles mode. Configure mode shows visual indicator. Panel slides in/out. All existing functionality preserved. ThemeApplier correctly merges endpoint-specific overrides when an endpoint is selected.
  </done>
</task>
...
7. Global style overrides applied to document CSS variables
8. When viewing a specific endpoint, endpoint-specific overrides merge on top of global
9. All existing rendering still works
</verification>
...
- ThemeApplier applies endpoint-specific overrides merged over global when endpoint is selected
